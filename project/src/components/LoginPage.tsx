...
import React, { useState } from 'react';
import { Chrome, Eye, EyeOff, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <button
        ...
        className="absolute top-4 left-4 z-10 bg-gray-700 text-white p-2 rounded-full hover:bg-gray-600 transition-all"
      >
      ...
      <div
        className={`transition-all duration-500 ease-in-out ${
          isSidebarOpen ? 'w-1/2' : 'w-16'
        } bg-[#1E1E1E] flex items-center justify-center p-4 overflow-hidden`}
      >
        {isSidebarOpen && (
          <div className="w-full max-w-md space-y-6">
            <h2 className="text-2xl font-bold text-white mb-6">Log in</h2>
      ...
      {/* Right side - Brand */}
      <div className={`transition-all duration-500 ease-in-out ${isSidebarOpen ? 'w-1/2' : 'w-full'} bg-[#1E3A8A] flex items-center justify-center p-8`}>
        <div className="text-center">
          <h1 className="text-4xl font-bold text-white mb-4">IIITDMJ ChatBot</h1>
          <p className="text-gray-300">An advanced virtual assistant</p>
          <div className="mt-4 flex justify-center gap-2">
            <span className="px-3 py-1 bg-[#E91E63] text-white rounded-full text-sm">
              Any confusion, try this bot
            </span>
          </div>
        </div>
      </div>
